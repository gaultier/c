...
int main() {
    ...
    ot_span_t* span_a = ot.span_create_root(trace_id, "span-a", OT_SK_CLIENT,
                                            "this is the first span");

    ot_span_t* span_b = ot.span_create_child_of(
        trace_id, "span-b", OT_SK_CLIENT, "this is the second span", span_a);
    ot.span_add_attribute(span_b, "service.name", "clone-gitlab-api");

    usleep(8);
    ot.span_end(span_b);
    ot_span_t* span_c = ot.span_create_child_of(
